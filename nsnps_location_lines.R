...
for(c in 1:length(chroms)){
  ...
  pdata$Oh1 = aggregate(nsnps13 ~ count, temp, mean)[[2]]
  
  pdata$Oh1u = aggregate(nsnps13 ~ count, temp, quantile, probs=0.975)[[2]]
  
  pdata$Oh1l = aggregate(nsnps13 ~ count, temp, quantile, probs=0.025)[[2]]
...
ydat = c(2)       #nsnps column number
tp = 0.2          #transparncy for points
plot(-100, -100, col="black", ylim = c(0, 3000), xlim=c(0,1), xlab="chromosome position", ylab="nsnps")
