library(scales)
setwd('/Volumes/jwillou/salmon_poolseq/bwa_mem/lien/depth/')

data1 = read.table("sumdepth_1_AP.txt", header=TRUE, sep="\t")
data2 = read.table("sumdepth_2_AP.txt", header=TRUE, sep="\t")
data3 = read.table("sumdepth_3_AP.txt", header=TRUE, sep="\t")

data = cbind(data1, data2$depth, data3$depth)
colnames(data) = c("chromosome", "location", "depth1", "depth2", "depth3")
...
for(c in 1:length(chroms)){
  ...
  pdata$Oh1 = aggregate(depth1 ~ count, temp, mean)[[2]]
  pdata$Oh2 = aggregate(depth2 ~ count, temp, mean)[[2]]
  pdata$Oh3 = aggregate(depth3 ~ count, temp, mean)[[2]]
  
  pdata$Oh1u = aggregate(depth1 ~ count, temp, quantile, probs=0.975)[[2]]
  pdata$Oh2u = aggregate(depth2 ~ count, temp, quantile, probs=0.975)[[2]]
  pdata$Oh3u = aggregate(depth3 ~ count, temp, quantile, probs=0.975)[[2]]
  
  pdata$Oh1l = aggregate(depth1 ~ count, temp, quantile, probs=0.025)[[2]]
  pdata$Oh2l = aggregate(depth2 ~ count, temp, quantile, probs=0.025)[[2]]
  pdata$Oh3l = aggregate(depth3 ~ count, temp, quantile, probs=0.025)[[2]]
  
  pdata$blockloc = seq(1, max(temp$location)+(windows*blocks), (windows*blocks))[1:nrow(pdata)]
...
plot(-100, -100, pch = 19, col="black", ylim = c(0, 100), xlim=c(0,1), xlab="chromosome position", ylab="coverage depth")
