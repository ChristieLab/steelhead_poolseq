library(scales)
#setwd("/scratch/snyder/j/jwillou/salmon_poolseq/bwa_mem/lien/repeat/")
setwd('/Volumes/jwillou/salmon_poolseq/bwa_mem/lien/repeat/')
data = read.table("mykiss_sumrepeatelements.txt", sep="", header=TRUE)
data$sumall = apply(data[,3:11], 1, sum)
...
#columns to plot
colplots = seq(3, 12, 1)
ymaxplots = c(25, 80, 5, 30, 100, 3, 5, 100, 15, 300)

#iterate over columns
for(i in 1:length(colplots)){
  ...
  for(c in 1:length(chroms)){
    ...
    pdata$Oh1 = aggregate(temp[,colplots[i]] ~ count, temp, mean)[[2]]
    pdata$Oh1u = aggregate(temp[,colplots[i]] ~ count, temp, quantile, probs=0.975)[[2]]
    pdata$Oh1l = aggregate(temp[,colplots[i]] ~ count, temp, quantile, probs=0.025)[[2]]
    
    pdata$blockloc = seq(1, max(temp$location)+(windows*blocks), (winstep*blocks))[1:nrow(pdata)]
    colnames(pdata) = c("group", "Oh1", "Oh1u", "Oh1l", "blockloc")
  ...
  xdat = 7          #location column number
  ydat = c(2)       #repeat element column numbers
  tp = 0.2          #transparncy for points
  plot(-100, -100, pch = 19, col="black", ylim = c(0, ymaxplots[i]), xlim=c(0,1), xlab="chromosome position", ylab=colnames(data)[colplots[i]])
  points(dat[, xdat], dat[, ydat[1]], pch = 19, col=alpha("darkorange", tp), bg=alpha("darkorange", tp), cex=0.5)
  ...
  for(s in 1:length(series)){
    ...
    outx <- mean(c(s1, s2))
    output <- cbind(outx, outy1)
    OUT <- rbind(OUT, output)
  }
  lines(OUT[, 1], OUT[, 2], lwd = 4, col="darkorange")
}
